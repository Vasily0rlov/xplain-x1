...
| P4-4 | **owner DAG review session** (soft-target validation, M-§4.5): nameability of Wine nodes; feedback recorded as issues | review notes in `docs/` | held | **🏁 M6 — first expert review of a live DAG** | ☐ | |
| P4-X | **exit criteria:** E4.1 bar met; owner review held | — | — | phase gate | ☐ | |

**P4 status:** artefact machinery complete and tested (P4-1..3 ☑; wine run emits model + concepts + dag.json/dot + certificate.json/md + provenance). **E4.1 NOT MET — escalated to owner (results review + principal decision):** Wine model beats the reference ceiling (fid ratio 1.017 vs v4 anchor 0.766) with fully μ-legible units on the known Wine core, but 0 concepts certify: at n=178 with collinear chemistry, restarts carve the correlated features into different legible mixes (Π 0.38–0.88) and CPSS on 53-row halves lacks power (π ≤ 0.6) — the honest-labelling machinery is working as designed; the question is whether the bar or the small-n statistical design should change. Options in FINDINGS P4.
