...
| **Taiwan Credit Default** (OpenML `default-of-credit-card-clients`) | 30,000 | 23 (mixed; PAY_* ordinal, SEX/MARRIAGE predicates) | 2 (22% default) | PAY_0 (most recent repayment status), LIMIT_BAL | ~0.20 pseudo-R² / ~0.82 acc (HGB) | 1 → d1 | ✅ **regulated flagship; full-power certificate** | **First genuinely regulated dataset (EU AI Act Annex III creditworthiness / SR 11-7).** Certifies `PAY_0` (Π=1.0, the literature's dominant driver) + `LIMIT_BAL`; **protected attributes (SEX/AGE/MARRIAGE) certify NOTHING** — the certificate carries an FDR-backed *non-reliance* statement. Low irreducible ceiling (credit default is noisy); ratio 0.861 = diffuse-frontier. Protected attrs kept as visible predicates for fair-lending review |
| **German Credit** (Statlog, OpenML `credit-g`) | 1,000 | 20 (readable nominals → predicates) | 2 (30% bad) | checking_status, duration, credit_history | ~0.20 pseudo-R² / ~0.71 acc | 1 → d1 | ✅ canonical benchmark (⚠️ n<floor → small-n regime) | **Household-name credit-scoring benchmark.** Certifies the classic Statlog triad (`checking_status=no checking` Π=1.0, `credit_history=critical`, `duration`); savings/purpose/amount land in the honest periphery at n=1,000. Demonstrates the small-n regime label in a regulated context; no protected attribute certifies |
