"""E1.1b — do the pressures work? (corrected instrument; milestone M2)

E1.1 as pre-registered ran on COMP2, whose intrinsic arity <= F_max saturates the
mu metric (OFF baseline 0.91): the fixture cannot discriminate.  E1.1b keeps the
bar and design but uses the discriminating ADD6 fixture (6 relevant inputs) and
the E1.3c-calibrated lambdas.  ADD6, fixed 2x12, ON vs OFF, 4 dev seeds each.
...
def main() -> int:
    ...
    out = {"experiment": "E1.1b", "dataset": DATASET, "widths": WIDTHS,
    ...
    save_json(ROOT / "experiments" / "results" / "e11b.json", out)
    v = out["verdict"]
    print(f"E1.1b: mu {v['mu_off']:.3f} -> {v['mu_on']:.3f} (gain {v['mu_gain']:+.3f}), "
